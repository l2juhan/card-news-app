...
| 재사용 대상 | 원본 경로 | 이식 방법 |
|-------------|-----------|-----------|
| HTML 템플릿 (189개) | `templates/` (12개 스타일) | 그대로 복사 |
...
│   │   │   ├── StyleSelector.tsx    # 12개 스타일 프리셋 선택
...
### Phase 2: 템플릿 시스템 연동 (Symlink)

**목표:** instagram-card-news의 리소스를 symlink로 연동하고, 렌더링 파이프라인 동작 확인

**Symlink 전략:**
개발 시 instagram-card-news의 리소스를 symlink로 참조하여 원본 업데이트를 자동 반영한다.
...
**작업 목록:**
- [x] `@anthropic-ai/claude-agent-sdk` 설치
- [x] instagram-card-news 스킬 재사용 확인
  - symlink된 `.claude/skills/card-news.md` (6단계 파이프라인: 리서치→팩트체크→카피→카피토론→렌더링→검토)
  - symlink된 `.claude/skills/edit-card-news.md` (수정 파이프라인)
  - symlink된 `.claude/skills/style-*.md` (12개 스타일 가이드)
  - Agent SDK에서 스킬 로딩 테스트
- [x] `src/main/claude.ts` 구현
  - `generateCardNews(topic, style, slideCount)` → Slide[]
  - `editCardNews(instruction)` → 수정 결과
  - `changeStyle(newStyle)` → 스타일 변경 결과
  - 스트리밍 진행 상태 콜백
  - 에러 핸들링 (SDK 초기화 실패, 연결 실패 등)
  - systemPrompt: { type: 'preset', preset: 'claude_code', append: ... }
  - settingSources: ['project'] 명시 (CLAUDE.md, 스킬 파일 로딩)
  - permissionMode: 'bypassPermissions' (Electron 앱 내 자동 실행)
- [x] `src/main/ipc.ts` 구현
  - `card-news:generate` 핸들러 → claude.ts → 결과 반환
  - `card-news:edit` 핸들러 → claude.ts → 단일 슬라이드 재렌더링
  - `card-news:direct-edit` 핸들러 → JSON 직접 수정 → render.js 재렌더링
  - `card-news:change-style` 핸들러 → claude.ts → 스타일 변경
  - `card-news:reorder-slides` 핸들러 → 순서 변경 → 재렌더링
  - `card-news:get-styles` 핸들러 → config.json 읽어 스타일 목록 반환
  - `card-news:export` 핸들러 → dialog.showOpenDialog → PNG 복사
  - 진행 상태 이벤트 Renderer로 전달
- [x] `src/preload/index.ts` 업데이트
  - contextBridge로 IpcApi 인터페이스에 맞춘 완전한 API 노출
- [x] 전체 파이프라인 테스트: 타입체크 + 빌드 통과 확인
...
- [x] `src/renderer/styles/global.css` - 기본 스타일, 폰트 로드 (Pretendard)
- [x] `src/renderer/stores/useCardNewsStore.ts` - Zustand 스토어
  ```
...
#### 4-2. 좌측 사이드바 (SideNav)
- [x] `SideNav.tsx` - 좌측 네비게이션 메뉴
  - 카드뉴스 만들기 (새 프로젝트)
  - 작업 목록 (이전 프로젝트 목록)
  - 설정 (스타일 기본값, 해상도 등)

#### 4-3. 중앙 패널 - 대화형 채팅 (ChatPanel)
- [x] `ChatPanel.tsx` - Claude Code 대화 인터페이스
  - 채팅 메시지 목록 (스크롤 영역)
  - 사용자 메시지 / AI 응답 버블 구분
  - 생성 중 스트리밍 텍스트 표시
- [x] `ChatMessage.tsx` - 개별 메시지 컴포넌트
  - 사용자 메시지: 주제 입력, 수정 요청 등
  - AI 메시지: 생성 완료 알림, 수정 결과 등
- [x] `ChatInput.tsx` - 하단 메시지 입력 영역
  - 텍스트 입력 (자연어: "AI 트렌드 주제로 7장 만들어줘", "3번 배경 파란색으로")
  - 전송 버튼 / Enter 키 전송
  - 스타일/장수 선택 옵션 (첫 생성 시)
- [x] `LoadingIndicator.tsx` - 생성/수정 중 타이핑 인디케이터

#### 4-4. 우측 패널 - Instagram 미리보기 (PreviewPanel)
- [x] `PreviewPanel.tsx` - 우측 미리보기 컨테이너
- [x] `PhoneMockup.tsx` - 아이폰 프레임 목업
  - 상단: 시간, 배터리 등 상태바
...
- [x] `SlideNavigator.tsx` - 폰 목업 하단 슬라이드 네비게이션
  - `« < 2/7 > »` 형태의 페이지네이션
...
- [x] `SlideEditor.tsx` - 인라인 편집 패널 (폰 목업 하단 또는 토글)
  - **텍스트 편집**: headline, body, subtext, emphasis 등 필드별 입력
  - **색상 편집**: accent_color 컬러 피커
  - **스타일 변경**: 드롭다운으로 12개 스타일 전환 (전체 슬라이드 일괄)
  - **슬라이드 타입 변경**: 현재 슬라이드의 type 전환 (content → content-stat 등)
  - **슬라이드 관리**: 추가 / 삭제 / 순서 변경 (드래그 앤 드롭) → Phase 5로 이관
  - "적용" 버튼 → JSON 수정 → IPC로 재렌더링 요청 → PNG 갱신
- [x] `ColorPicker.tsx` - accent_color 선택 컴포넌트
  - 프리셋 색상 팔레트 + 커스텀 HEX 입력
- [x] `src/main/ipc.ts`에 직접 편집용 핸들러 추가 (Phase 3에서 구현 완료)
...
#### 4-6. IPC 훅
- [x] `src/renderer/hooks/useIpc.ts`
  - window.api를 래핑하는 React 훅
  - IPC 이벤트 리스너 등록/해제 (useEffect)
- [x] `src/renderer/hooks/useChat.ts`
...
#### 5-1. 내보내기
- [x] `ExportButton.tsx` 구현
  - 전체 슬라이드 PNG 일괄 내보내기
  - 해상도 선택 메뉴 (4:5, 1:1)
  - 저장 위치 선택 다이얼로그 (Electron dialog.showOpenDialog)
- [x] `card-news:export` IPC 핸들러 (Phase 3에서 구현 완료)
- [x] `src/renderer/hooks/useExport.ts` 훅 구현

#### 5-2. UX 개선
- [x] 슬라이드 드래그 앤 드롭 순서 변경 (SlideGrid)
  - @dnd-kit/core + @dnd-kit/sortable 사용
  - 순서 변경 시 슬라이드 번호 자동 갱신
- [x] 생성 중 스트리밍 진행 표시
  - 프로그레스 바 + 단계별 아이콘 (리서치, 팩트체크, 카피, 렌더링 등)
  - Claude SDK 스트리밍 → IPC PROGRESS 이벤트 (Phase 3에서 구현)
- [x] 에러 핸들링 개선
  - Claude Code 미설치 감지 (ENOENT+claude) → 설치 안내 메시지
  - 에러 메시지에 재시도 버튼 추가
- [x] 키보드 단축키
  - `Cmd+E`: 내보내기
  - `Esc`: 포커스 해제
  - `←/→`: 슬라이드 탐색 (Phase 4에서 구현)
